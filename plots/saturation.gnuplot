...
ARGS = "-N %d --noise=2 --nions=%d --pulsewidth=1.0 --gain=50.0 --gain-sigma=100.0 --width=%g --rate=%g"
aW = 0.4
N = 1
rate = 2.0
...
do for [i=1:2] {
    ...
    s = gain*sqrt(n)
    m = gain*n
    set arrow 1 from 1000,0.0 to 1000,nd(1000,gain*n,gain*sqrt(n)) nohead lw 5 lc rgb "red"
    set arrow 2 from m,0.0 to m,nd(gain*n,gain*n,gain*sqrt(n)) nohead lw 3
    set arrow 3 from m+s*2,0.0 to m+s*2,nd(m+s*2,gain*n,gain*sqrt(n)) nohead lw 1
    set label 4 at m+s*2,nd(m+s*2,gain*n,gain*sqrt(n)) offset -1,1 "$2\\sigma$"
    plot nd(x,gain*n,gain*sqrt(n)) lw 5 title sprintf("n-ions=%d, %g mV/ion, $m$=%g, $\\sigma$=%g", n, gain, gain*n, gain*sqrt(n))
}
